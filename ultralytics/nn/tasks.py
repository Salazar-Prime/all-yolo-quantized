...
def guess_model_family(model):
    ...
    if isinstance(model, (str, Path)):
        ...
        # (incl. .engine), e.g. yolo27n-detr / yolo27l-detr / yolo27xxl-detr -> yolo27<scale>detr. The bare yolo27l
        # name is matched too since the shipped YOLO27 l-scale is a DEIMDecoder model, but only at the end of the
        # stem: punctuation is already stripped, so an unanchored match would also claim yolo27l-seg and yolo27l-cls,
        ...
        # not match).
        if "yolodetr" in stem or re.search(r"yolo\d+[a-z]*detr|yolo27l$", stem):
            return "yolodetr"
