...
def deployment_for(model, deployments=DEPLOYMENTS):
    return next(
        name
        for name, device in deployments.items()
        if model in device.get("models", ()) or model.startswith(tuple(device.get("model_prefixes", ())))
    )
...
def main():
    ...
    parser.add_argument("--protocol", type=Path, default=ROOT / "exp5/protocol.json")
    parser.add_argument("--device")
    args = parser.parse_args()
    protocol = json.loads(args.protocol.read_text())
    deployments = protocol["deployments"]
    args.device = args.device or next(iter(deployments))
    if args.device not in deployments:
        parser.error("Device is not present in the selected protocol")
    ...
    output = ROOT / protocol["experiment"] / "runs" / args.run
    ...
    cases = list(csv.DictReader((ROOT / protocol["model_manifest"]).open()))
    cases = sorted(
        (case for case in cases if deployment_for(case["model"], deployments) == args.device),
        key=lambda case: int(case["checkpoint_bytes"]),
    )
    device = deployments[args.device]
    HOST, XAVIER = device["ssh_alias"], device["root"]
    telemetry = XAVIER + "/" + protocol["experiment"] + "/runs/" + args.run + "/telemetry.jsonl"
    ...
    for case, group in queue:
        ...
        if not variants:
            continue
        xavier_dir = str(Path(telemetry).parent / model)
        local.mkdir(parents=True, exist_ok=True)
        if "source_archive" in protocol:
            prepared = ROOT / protocol["source_archive"] / model
        else:
            ...
            prepared = local
        if not (prepared / "reference.json").exists():
            (local / "preparation-failed.json").write_text(json.dumps(case, indent=2) + "\n")
            continue
        remote(HOST, "mkdir -p " + shlex.quote(xavier_dir))
        sources = {variant.split("_")[0] if variant.startswith(("ptq", "qat")) else "fp32" for variant in variants}
        for name in [source + ".onnx" for source in sorted(sources)] + ["reference.json"]:
            if (prepared / name).exists():
                transfer(str(prepared / name), HOST + ":" + xavier_dir + "/")
        if "source_archive" in protocol:
            inputs = "\n".join(
                json.loads((prepared / (source + ".json")).read_text())["sha256"] + "  " + source + ".onnx"
                for source in sorted(sources)
            )
            remote(
                HOST,
                "cd {} && printf '%s\\n' {} >inputs.sha256 && sha256sum -c inputs.sha256".format(
                    shlex.quote(xavier_dir), shlex.quote(inputs)
                ),
            )
        remote(
            HOST,
        ...
        subprocess.run(
            ["python3", str(ROOT / "exp5/summarize.py"), str(output), "--protocol", str(args.protocol)], check=True
        )
        archive_lock.close()
        print("Archived and removed Xavier copy of " + model, flush=True)
    remote(HOST, "touch " + shlex.quote(str(Path(telemetry).with_suffix(".stop"))))
